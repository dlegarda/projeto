...
void Slic::init_data(IplImage *image) {
 ...
 printf("ancho %i alto %i Hx=%i Hy=%i %f\n",sz.width,sz.height, image->width, image->height, step);
    /* Initialize the centers and counters. */
    for (int i = step; i < sz.width - step/2; i += step) {
        for (int j = step; j < sz.height - step/2; j += step) {
        ...
        for (int j = 0; j < (int) centers.size(); j++) {
            ...
            for (int k = centers[j][3] - step; k < centers[j][3] + step; k++) {
                for (int l = centers[j][4] - step; l < centers[j][4] + step; l++) {
                    ...
                    if (k >= 0 && k < sz.width && l >= 0 && l < sz.height) {
...
void Slic::create_connectivity(IplImage *image) {
    ...
    if (centers.size()!=0){
    const int lims = (sz.width * sz.height) / ((int)centers.size());
    }
    const int lims =2;
...
CvSize sz=cvGetSize(image);
    for (int i = 0; i < (int) centers.size(); i++) {
        ...
        centros[i][0]=centers[i][3];
        centros[i][1]=centers[i][4];
    }
}
